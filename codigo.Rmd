...
```{r Calcular medias filtrando por consumo}
...
```{r Tablas de Contingencias Categoricas Cannabis}
...
```{r Tablas de Contingencias Categoricas Coke}
...
```{r Tablas de Contingencias Categoricas Heroin}
...
```{r Tablas de Contingencias Categoricas Meth}
...
```{r Tablas de Contingencias Categoricas Nicotine}
...
### Pruebas de Chi Cuadrado y V cramer

Primeramente, para analizar como variable cualitativa los resultados de los test de 
personalidad, procedemos a crear intervalos en estas variables para usarlos en las 
tablas de contingencia.

```{r Discretizar los Scores}
oscore <- cut(base$Oscore,breaks = 3)
nscore <- cut(base$Nscore,breaks = 3)
escore <- cut(base$Escore,breaks = 3)
ascore <- cut(base$AScore,breaks = 3)
cscore <- cut(base$Cscore,breaks = 3)
```

A continuación se presenta el proceso de resultados. Para entender estos resultados
vease el título de cada chunck, el cual ofrece la variable que se está analizando.
Cada chunck realiza el mismo proceso: primero se crea la tabla de contingencia de frecuencias, luego a esa tabla se le aplica la prueba Chi-cuadrado y por último se
calcula el coeficiente V de Cramer. Se analiza cada variable en el mismo orden del Latex.

```{r Resultados de O-score}
...
La prueba Chi-cuadrado y el resultado de la V de cramer nos ayudan a establecer una relación entre las variables de la base y el consumo de drogas, mediante el rechazo de la hipótesis nula de que son independientes y el posterior cálculo del coeficiente para ver la fuerza de la relación. 
